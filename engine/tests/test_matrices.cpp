...
TEST(Matrix, get) {
    Matrix<3> matrix({1, 4, 7, 2, 5, 8, 3, 6, 9});
...
TEST(Matrix, equals) {
    ...
    Matrix<3> matrix2({1, 4, 7, 2, 5, 8, 3, 6, 9});

    Matrix<3> matrix3({1, 4, 7, 2, 8, 8, 3, 6, 9});
    ASSERT_EQ(matrix, matrix2);
    ASSERT_TRUE(matrix != matrix3);
}

TEST(Matrix, multiply4x4) {
    Matrix<4> matrix({1, 2, 3, 4, 5, 6, 7, 8, 9, 8, 7, 6, 5, 4, 3, 2});

    Matrix<4> matrix2({-2, 1, 2, 3, 3, 2, 1, -1, 4, 3, 6, 5, 1, 2, 7, 8});
...
TEST(Matrix, transpose) {
    Matrix<4> matrix({0, 9, 3, 0, 9, 8, 0, 8, 1, 8, 5, 3, 0, 0, 5, 8});

    Matrix<4> expected({0, 9, 1, 0, 9, 8, 8, 0, 3, 0, 5, 5, 0, 8, 3, 8});

    ASSERT_EQ(expected, transpose(matrix));
}

TEST(Matrix, submatrix) {
    Matrix<3> matrix({1, 5, 0, -3, 2, 7, 0, 6, -3});

    Matrix<2> expected({-3, 2, 0, 6});

    Matrix<2> expected2({2, 7, 6, -3});
...
TEST(Matrix, minor) {
    Matrix<3> matrix({3, 5, 0, 2, -1, 7, 6, -1, 5});
...
TEST(Matrix, determinant) {
    Matrix<2> matrix({1, 5, -3, 2});
    ASSERT_EQ(17, determinant(matrix));

    Matrix<3> matrix1({1, 2, 6, -5, 8, -4, 2, 6, 4});
    ASSERT_EQ(-196, determinant(matrix1));

    Matrix<4> matrix2({-2, -8, 3, 5, -3, 1, 7, 3, 1, 2, -9, 6, -6, 7, 7, -9});
    ASSERT_EQ(-4071, determinant(matrix2));
}

TEST(Matrix, is_invertable) {
    Matrix<4> matrix({6, 4, 4, 4, 5, 5, 7, 6, 4, -9, 3, -7, 9, 1, 7, -6});

    Matrix<4> matrix2({-4, 2, -2, -3, 9, 6, 2, 6, 0, -5, 1, -5, 0, 0, 0, 0});
    ASSERT_TRUE(is_invertable(matrix));
    ASSERT_FALSE(is_invertable(matrix2));
}

TEST(Matrix, inverse) {
    Matrix<4> matrix({-5, 2, 6, -8, 1, -5, 1, 8, 7, 7, -6, -7, 1, -3, 7, 4});

    Matrix<4> expected({0.21805, 0.45113, 0.24060, -0.04511, -0.80827, -1.45677,
                        -0.44361, 0.52068, -0.07895, -0.22368, -0.05263,
