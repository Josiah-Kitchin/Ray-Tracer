...
TEST(Matrix, equals) { 
    ...
    Matrix<3> matrix2({1, 4, 7,
                    2, 5, 8,
                    3, 6, 9});

    Matrix<3> matrix3({1, 4, 7,
                    2, 8, 8,
                    3, 6, 9});
...
TEST(Matrix, multiply4x4) { 
    Matrix<4> matrix({1, 2, 3, 4, 
                  5, 6, 7, 8, 
                  9, 8, 7, 6, 
                  5, 4, 3, 2});

    Matrix<4> matrix2({-2, 1, 2, 3, 
                   3, 2, 1, -1, 
                   4, 3, 6, 5, 
                   1, 2, 7, 8});

    Matrix<4> expected({20, 22, 50, 48, 
                    44, 54, 114, 108, 
                    40, 58, 110, 102, 
                    16, 26, 46, 42});
...
TEST(Matrix, transpose) { 
    Matrix<4> matrix({0, 9, 3, 0, 
                  9, 8, 0, 8, 
                  1, 8, 5, 3, 
                  0, 0, 5, 8}); 
    
    Matrix<4> expected({0, 9, 1, 0, 
                    9, 8, 8, 0, 
                    3, 0, 5, 5, 
                    0, 8, 3, 8});
...
TEST(Matrix, submatrix) { 
    Matrix<3> matrix({ 1, 5,  0, 
                  -3, 2,  7, 
                   0, 6, -3}); 
    
    Matrix<2> expected({-3, 2,
                     0, 6}); 
    
    Matrix<2> expected2({2,  7, 
                     6, -3});
...
TEST(Matrix, determinant) { 
    ...
    Matrix<3> matrix1({ 1, 2,  6, 
                  -5, 8, -4, 
                   2, 6,  4});
    ASSERT_EQ(-196, determinant(matrix1)); 

    Matrix<4> matrix2({-2, -8,  3,  5, 
                   -3,  1,  7,  3,
                    1,  2, -9,  6,
                   -6,  7,  7, -9}); 
...
TEST(Matrix, is_invertable) { 
    Matrix<4> matrix({6, 4, 4, 4, 
                  5, 5, 7, 6, 
                  4, -9, 3, -7, 
                  9, 1, 7, -6});

    Matrix<4> matrix2({-4, 2, -2, -3, 
                   9, 6, 2, 6, 
                   0, -5, 1, -5, 
                   0, 0, 0, 0});
...
TEST(Matrix, inverse) { 
    Matrix<4> matrix ({-5, 2, 6, -8, 
                   1, -5, 1, 8, 
                   7, 7, -6, -7, 
                   1, -3, 7, 4});

    Matrix<4> expected({0.21805,  0.45113, 0.24060, -0.04511, 
